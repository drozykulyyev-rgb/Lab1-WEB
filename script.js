...
function getR() {
    for (const checkbox of checkboxes) {
        if (checkbox.checked) return Number(checkbox.value);
    }
}
...
function validate(input, min, max, errorId) {
    ...
    if (text === '' || !Number.isFinite(value)) error = 'Введите число';
    else if (value < min || value > max) error = `Допустимо от ${min} до ${max}`;
...
function checkHit(x, y, r) {
    const rectangle = x >= 0 && x <= r && y >= 0 && y <= r;
    const circle = x <= 0 && y <= 0 && x * x + y * y <= r * r / 4;
    const triangle = x >= 0 && y <= 0 && y >= x - r;
...
function showHistory() {
    ...
    for (const item of history) {
        ...
        row.insertCell().textContent = new Date(item.time).toLocaleString('ru-RU');
    }
}
...
checkboxes.forEach(function (checkbox) {
    checkbox.addEventListener('change', function () {
        if (this.checked) {
            checkboxes.forEach(item => { if (item !== this) item.checked = false; });
        } else if (!getR()) {
            this.checked = true;
        }
        currentPoint = null;
...
form.addEventListener('submit', function (event) {
    ...
    const x = validate(document.getElementById('x'), -3, 5, 'x-error');
    const y = validate(document.getElementById('y'), -3, 3, 'y-error');
    const r = getR();

    if (x === null || y === null || !r) return;

    const point = { x: x, y: y, r: r, hit: checkHit(x, y, r), time: new Date().toISOString() };
    currentPoint = point;
    history.unshift(point);
    localStorage.setItem('points22721', JSON.stringify(history));
    showHistory();
    draw(point);
});
